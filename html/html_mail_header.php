<!-- start of $Id: html_mail_header.php,v 1.23 2002/03/24 17:08:02 wolruf Exp $ -->
<?php
if ($verbose == 1 && $conf->use_verbose == true)
    echo '<tr><td class="mail"><a href="' . $PHP_SELF . '?action=aff_mail&amp;mail=' . $mail . '&amp;verbose=0&amp;lang=' . $lang . '&amp;sort=' . $sort . '&amp;sortdir=' . $sortdir . '&amp;folder='.$folder .'" class="mail">' . $html_remove_header . '</a></td>';
elseif ($conf->use_verbose == true)
    echo '<tr><td class="mail"><a href="' . $PHP_SELF . '?action=aff_mail&amp;mail=' . $mail . '&amp;verbose=1&amp;lang=' . $lang . '&amp;sort=' . $sort . '&amp;sortdir=' . $sortdir . '&amp;folder='.$folder .'" class="mail">' . $html_view_header . '</a></td>';
else
    echo '<tr><td>&nbsp;</td>';

if (($content['prev'] != '') && ($content['prev'] != 0))
    $prev = '<a href="' . $PHP_SELF . '?action=aff_mail&amp;mail=' . $content['prev'] . '&amp;verbose=' . $verbose . '&amp;lang=' . $lang . '&amp;sort=' . $sort . '&amp;sortdir=' . $sortdir . '&amp;folder='.$folder .'"><img src="themes/' . $theme . '/img/left_arrow.gif" alt="' . $alt_prev . '" title="' . $alt_prev . '" border="0" /></a>';
if (($content['next'] != '') && ($content['next'] != 0))
    $next =  '<a href="' . $PHP_SELF . '?action=aff_mail&amp;mail=' . $content['next'] . '&amp;verbose=' . $verbose . '&amp;lang=' . $lang . '&amp;sort=' . $sort . '&amp;sortdir=' . $sortdir . '&amp;folder='.$folder .'"><img src="themes/' . $theme . '/img/right_arrow.gif" alt="' . $alt_next . '" title="' . $alt_next . '" border="0" /></a>';
?>    
...
<!-- end of $Id: html_mail_header.php,v 1.23 2002/03/24 17:08:02 wolruf Exp $ -->
