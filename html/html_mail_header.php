<!-- start of $Id: html_mail_header.php,v 1.28 2002/05/14 18:46:43 rossigee Exp $ -->
<?php
// Show/hide header
$verbose = (isset($_REQUEST['verbose']) && $_REQUEST['verbose'] == 1) ? '1' : '0';
if ($conf->use_verbose)
    echo '<tr><td class="mail"><a href="' . $PHP_SELF . '?action=aff_mail&amp;mail=' . $content['msgnum'] . '&amp;verbose=' . $verbose . '" class="mail">' . $html_remove_header . '</a></td>';
else
    echo '<tr><td>&nbsp;</td>';

if (($content['prev'] != '') && ($content['prev'] != 0))
    $prev = '<a href="' . $PHP_SELF . '?action=aff_mail&amp;mail=' . $content['prev'] . '&amp;verbose=' . $verbose . '"><img src="themes/' . $theme . '/img/left_arrow.gif" alt="' . $alt_prev . '" title="' . $alt_prev . '" border="0" /></a>';
if (($content['next'] != '') && ($content['next'] != 0))
    $next =  '<a href="' . $PHP_SELF . '?action=aff_mail&amp;mail=' . $content['next'] . '&amp;verbose=' . $verbose . '"><img src="themes/' . $theme . '/img/right_arrow.gif" alt="' . $alt_next . '" title="' . $alt_next . '" border="0" /></a>';
?>    
...
<!-- end of $Id: html_mail_header.php,v 1.28 2002/05/14 18:46:43 rossigee Exp $ -->
