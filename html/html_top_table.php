...
if ($pop->is_imap()) {
    if ($pop->get_folder_count() > 1) {
        $fldr_line = "<form method=\"post\" action=\"action.php\"><div><label for=\"folder\">$html_other_folders:</label>  \n";
    ...
    if ($_SESSION['nocc_folder'] == 'INBOX') {
        $reapply_filters = '<form method="post" action="action.php"><div>'.
                          ...
                          <?php
                            foreach ($conf->column_order as $column) { //For all columns...
                                ...
                                if ($column_title != '') { //If we have a column title...
                                    echo '<a href="action.php?sort='.$column.'&amp;sortdir='.$new_sortdir.'">'.$column_title.'</a>';
                                    if ($_SESSION['nocc_sort'] == $column) {
                                        echo '&nbsp;';
                                        echo '<a href="action.php?sort='.$column.'&amp;sortdir='.$new_sortdir.'">';
